...
class XrayLuminosity() :
	...
	def __init__(self, Lbox=2500.0 , boxDir=os.environ['MD04'], snl=[], Hbox = 67.77 , Melement = 23593750000.0 ):
	...
	def Phi_stellar_mass_mmin(self, stellar_mass, redshift, mmin):
		"""
		Integrates the bivariate distribution function on logM (equation 8). Gives the host galxy stellar mass function (AGN HGMF). :math:`dN/dVdlog\lambda`
		:param logM: stellar mass
		:param z: redshift
		...
		if mmin<33:
		  integrand = lambda lambda_SAR, logM, z : self.psi_log( n.log10(lambda_SAR), logM, z)/(n.log(10)*lambda_SAR)
		  return quad(integrand, 10**mmin, 10**33, args=(stellar_mass, redshift))[0] + quad(integrand, 10**33, 10**34, args=(stellar_mass, redshift))[0] + quad(integrand, 10**34, 10**35, args=(stellar_mass, redshift))[0]+quad(integrand, 10**35, 10**36, args=(stellar_mass, redshift))[0]
		elif mmin<34:
		  integrand = lambda lambda_SAR, logM, z : self.psi_log( n.log10(lambda_SAR), logM, z)/(n.log(10)*lambda_SAR)
		  return quad(integrand, 10**mmin, 10**34, args=(stellar_mass, redshift))[0] + quad(integrand, 10**34, 10**35, args=(stellar_mass, redshift))[0]+quad(integrand, 10**35, 10**36, args=(stellar_mass, redshift))[0]
		elif mmin<35:
		  integrand = lambda lambda_SAR, logM, z : self.psi_log( n.log10(lambda_SAR), logM, z)/(n.log(10)*lambda_SAR)
		  return quad(integrand, 10**mmin, 10**35, args=(stellar_mass, redshift))[0]+quad(integrand, 10**35, 10**36, args=(stellar_mass, redshift))[0]
		elif mmin<36:
		  integrand = lambda lambda_SAR, logM, z : self.psi_log( n.log10(lambda_SAR), logM, z)/(n.log(10)*lambda_SAR)
		  return quad(integrand, 10**mmin, 10**36, args=(stellar_mass, redshift))[0]
	
	
	
	def Phi_stellar_mass_33(self, stellar_mass, redshift):
		"""
		Integrates the bivariate distribution function on logM (equation 8). Gives the host galxy stellar mass function (AGN HGMF). :math:`dN/dVdlog\lambda`
		:param logM: stellar mass
		:param z: redshift
		"""
		integrand = lambda lambda_SAR, logM, z : self.psi_log( n.log10(lambda_SAR), logM, z)/(n.log(10)*lambda_SAR)
		return quad(integrand, 10**33, 10**34, args=(stellar_mass, redshift))[0] + quad(integrand, 10**34, 10**35, args=(stellar_mass, redshift))[0]+quad(integrand, 10**35, 10**36, args=(stellar_mass, redshift))[0]
	
	def Phi_stellar_mass_34(self, stellar_mass, redshift):
		"""
		Integrates the bivariate distribution function on logM (equation 8). Gives the host galxy stellar mass function (AGN HGMF). :math:`dN/dVdlog\lambda`
		:param logM: stellar mass
		:param z: redshift
		"""
		integrand = lambda lambda_SAR, logM, z : self.psi_log( n.log10(lambda_SAR), logM, z)/(n.log(10)*lambda_SAR)
		return quad(integrand, 10**34, 10**35, args=(stellar_mass, redshift))[0]+quad(integrand, 10**35, 10**36, args=(stellar_mass, redshift))[0]
	
	def Phi_stellar_mass_35(self, stellar_mass, redshift):
		"""
		Integrates the bivariate distribution function on logM (equation 8). Gives the host galxy stellar mass function (AGN HGMF). :math:`dN/dVdlog\lambda`
		:param logM: stellar mass
		:param z: redshift
		"""
		integrand = lambda lambda_SAR, logM, z : self.psi_log( n.log10(lambda_SAR), logM, z)/(n.log(10)*lambda_SAR)
		return quad(integrand, 10**35, 10**36, args=(stellar_mass, redshift))[0]
