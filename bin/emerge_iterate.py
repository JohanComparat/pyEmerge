...
id_map = n.hstack(n.vstack(n.array([n.argwhere(f1['/halo_data/id'].value == pix_id) for pix_id in f0_ids_f1 ]) ))
...
print("N halos in f0 ", len(f0['/halo_data/desc_scale'].value), len(list( set(f0_ids_f1))))
...
def compute_qtys_existing_halos(f_snap, f_previous, selection):
	# computing dMdt for the halo
	mvir_dot = (f_snap['/halo_data/mvir'].value[selection]-f_previous['/halo_data/mvir'].value) / (f_snap.attrs['age_yr'] - f_previous.attrs['age_yr'])
	rvir_dot = (f_snap['/halo_data/rvir'].value[selection]-f_previous['/halo_data/rvir'].value) / (f_snap.attrs['age_yr'] - f_previous.attrs['age_yr'])
	c = f_snap['/halo_data/rvir'].value[selection] / f_snap['/halo_data/rs'].value[selection]
	rho_nfw = f_snap['/halo_data/mvir'].value[selection] / (f_snap['/halo_data/rs'].value[selection]**3. * 4. * n.pi * c * (1+c)**2. * (n.log(1.+c)-c/(1.+c)))
	pseudo_evolution_correction = 4.*n.pi*f_snap['/halo_data/rvir'].value[selection] *f_snap['/halo_data/rvir'].value[selection] * rvir_dot * rho_nfw
	dMdt = mvir_dot - pseudo_evolution_correction
	...
	m_icm = f_previous['/emerge_data/m_icm'].value
	...
	# evaluate stellar mass 
	star_formation_rate = dmdt_star * (1. - f_lost) + dmdt_star_accretion 
	stellar_mass = star_formation_rate * (f_snap.attrs['age_yr']-f_previous.attrs['age_yr']) + f_previous['/emerge_data/stellar_mass'].value
	...
	t_quench = tau_quenching( f_previous['/emerge_data/stellar_mass'].value, t_dynamical[selection] )
	...
	if len(quench_1.nonzero()[0])>0:
		print("quenching1")
		star_formation_rate[quench_1] = n.ones_like(star_formation_rate[quench_1])*f_previous['/emerge_data/stellar_mass'].value[quench_1]
		stellar_mass[quench_1] = star_formation_rate[quench_1] * (f_snap.attrs['age_yr']-f_previous.attrs['age_yr']) + f_previous['/emerge_data/stellar_mass'].value[quench_1]
	...
	if len(quench_2.nonzero()[0])>0:
		...
		stellar_mass[quench_2] = f_previous['/emerge_data/stellar_mass'].value[quench_2]
	...
	if len(stripping_1.nonzero()[0])>0:
		print("stripping1")
		m_icm[stripping_1] += f_previous['/emerge_data/stellar_mass'].value[stripping_1]
	...
	if len(stripping_2.nonzero()[0])>0:
		print("stripping2")
		m_icm[stripping_2] += f_previous['/emerge_data/stellar_mass'].value[stripping_2]
	...
	will_merge = (f_previous['/halo_data/Time_to_future_merger'].value>0)
