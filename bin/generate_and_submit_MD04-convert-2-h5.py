...
aexp, redshift, age_yr, rho_crit, delta_vir = n.loadtxt("/u/joco/data/MD/MD_1.0Gpc/hlists_MD_0.4Gpc.ascii", unpack=True)
...
for snap_id in snap_ids:
    ...
    command = "python MD04-convert-2-h5.py " + snap_names[snap_id] +" "+str(aexp[snap_id])+" "+str( redshift[snap_id])+" "+str( age_yr[snap_id])+" "+str( rho_crit[snap_id])+" "+str( delta_vir[snap_id])
    print(command)
    f.write(command)
    f.close()
