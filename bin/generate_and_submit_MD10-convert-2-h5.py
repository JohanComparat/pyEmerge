...
#
# @ error = /u/joco/run_monitor/md_h5_"""+snap_name+""".err.$(jobid)
# @ output = /u/joco/run_monitor/md_h5_"""+snap_name+""".out.$(jobid)
# @ job_type = parallel
...
for snap_id in snap_ids:
    print(snap_id)
    batch_file = os.path.join(batch_dir, "h5_"+snap_names[snap_id]+"_md.sh")
    f=open( batch_file, 'w')
    f.write(main_text(snap_names[snap_id]))
    command = "python MD10-convert-2-h5.py " + snap_names[snap_id] +" "+str(aexp[snap_id])+" "+str( redshift[snap_id])+" "+str( age_yr[snap_id])+" "+str( rho_crit[snap_id])+" "+str( delta_vir[snap_id])
    print(command)
    f.write(command)
    f.close()
