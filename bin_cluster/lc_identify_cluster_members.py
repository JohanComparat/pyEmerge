...
for jj in n.arange(len(position_BCG)):
  ...
  radial_distance = n.sum((position_BCG[jj]-treeData.data[out[jj]])**2., axis=1)**0.5 / radial_distance_2_member[jj] * 10.
...
Mag_abs_i = lambda x : - 22. - 5.9*( x*(1+x*x) - 0.8)
Mag_abs_r = lambda x : - 22. - 5.9*( x*(1+x*x) - 0.3)
...
fg['/cluster_galaxies'].create_dataset('d_cluster_center', data = radial_separation)
  ...
  ##fg['/cluster_galaxies'].create_dataset('mag_r', data = mag_r )
  #ds = halo_data.create_dataset('galaxy_mag_r', data = mag_r )
